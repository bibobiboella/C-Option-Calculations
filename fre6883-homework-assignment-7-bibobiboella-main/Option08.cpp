...
namespace fre {
    ...
    double OptionCalculation::PriceByCRR(const BinomialTreeModel &Model) {
        ...
        for (int n = N - 1; n >= 0; n--) {
            for (int i = 0; i <= n; i++) {
                Price[i] =
                    (q * Price[i + 1] + (1 - q) * Price[i]) / Model.GetR();
            }
        }
        return Price[0];
    }
    ...
    BinLattice<double>& XStockTree, BinLattice<double>& YMoneyMarketTree)
    {
     double q = Model.RiskNeutProb();
     int N = pOption->GetN();
     PriceTree.SetN(N);
     XStockTree.SetN(N);
     YMoneyMarketTree.SetN(N);
     //vector<double> Price(N + 1);
     for (int i = 0; i <= N; i++)
     {
         //initialization
        PriceTree.SetNode(N, i, pOption->Payoff(Model.CalculateAssetPrice(N, i)));
        XStockTree.SetNode(N, i, 0);
        YMoneyMarketTree.SetNode(N, i, 0);
     }
     for (int n = N - 1; n >= 0; n--)
     {
         for (int i = 0; i <= n; i++)
         {
            //pricetree = tree for option pricing
            PriceTree.SetNode(n, i, 
            (q * PriceTree.GetNode(n + 1, i + 1) + 
            (1 - q) * PriceTree.GetNode(n + 1, i)) / Model.GetR());
            //according to PPT formula
            XStockTree.SetNode(n, i,
            (PriceTree.GetNode(n + 1, i + 1) - PriceTree.GetNode(n + 1, i)) / 
            (Model.CalculateAssetPrice(n + 1, i + 1) - Model.CalculateAssetPrice(n + 1, i))
            );
            
            YMoneyMarketTree.SetNode(n, i,
            (PriceTree.GetNode(n + 1, i) - 
            XStockTree.GetNode(n, i) * Model.CalculateAssetPrice(n + 1, i)) / Model.GetR()
            );
